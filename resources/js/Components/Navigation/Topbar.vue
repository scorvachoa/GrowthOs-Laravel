    ...
    <header class="sticky top-0 z-20 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 px-8 py-4 flex items-center justify-between">
        ...
        <div class="relative" v-if="isSuperAdmin && companies.length > 0" ref="switcherRef">
            <button @click="toggleSwitcher"
                ...
                <div>
                    <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ activeCompany?.name || 'Seleccionar empresa' }}</p>
            ...
            <div v-if="switcherOpen"
                ...
                <button v-for="company in companies" :key="company.id" @click="switchCompany(company.id)"
                    ...
                    <svg v-if="company.id === activeCompany?.id" class="w-4 h-4 ml-auto text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="flex items-center gap-4">
            ...
            <Link
                href="/profile"
                class="flex items-center gap-3 hover:bg-gray-100 dark:hover:bg-gray-800 px-3 py-2 rounded-xl transition"
            >
            ...
            <button
                @click="logout"
                class="px-4 py-2 rounded-xl bg-red-600 hover:bg-red-700 text-white transition"
            >
                Logout
            </button>
