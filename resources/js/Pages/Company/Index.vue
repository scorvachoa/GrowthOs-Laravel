...
function submitCompany() {
    ...
    router.post('/company/update', form, {
        preserveScroll: true,
        onSuccess: () => {
            orgForm.value.remove_logo = false
        },
...
function deleteChannel(channel) {
    if (confirm(`Eliminar el canal "${channel.name}"?`)) {
        router.post(`/company/channels/${channel.id}/delete`, { preserveScroll: true })
    }
}
</script>
...
<template>
    <AppLayout>
        <div class="max-w-5xl mx-auto space-y-8">
            <div>
                <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Empresa</h1>
